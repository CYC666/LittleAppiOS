...
@interface CalendarScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
- (NSMutableArray *)monthArray {
    ...
    if (_monthArray == nil) {
        ...
        [_monthArray addObject:[[CalendarMonth alloc] initWithDate:previousMonthDate]];
        [_monthArray addObject:[[CalendarMonth alloc] initWithDate:_currentMonthDate]];
        [_monthArray addObject:[[CalendarMonth alloc] initWithDate:nextMonthDate]];
...
- (void)setupCollectionViews {
    ...
    [_collectionViewL registerClass:[CalendarCell class] forCellWithReuseIdentifier:kCellIdentifier];
    ...
    [_collectionViewM registerClass:[CalendarCell class] forCellWithReuseIdentifier:kCellIdentifier];
    ...
    [_collectionViewR registerClass:[CalendarCell class] forCellWithReuseIdentifier:kCellIdentifier];
...
- (void)notifyToChangeCalendarHeader {
    ...
    CalendarMonth *currentMonthInfo = self.monthArray[1];
...
- (void)refreshToCurrentMonth {
    ...
    [self.monthArray addObject:[[CalendarMonth alloc] initWithDate:previousMonthDate]];
    [self.monthArray addObject:[[CalendarMonth alloc] initWithDate:_currentMonthDate]];
    [self.monthArray addObject:[[CalendarMonth alloc] initWithDate:nextMonthDate]];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionViewL) { // 上一个月展示数据
        ...
        CalendarMonth *monthInfo = self.monthArray[0];
        ...
        CalendarMonth *monthInfo = self.monthArray[1];
        ...
        // 补上前后月的日期，淡色显示
            ...
            CalendarMonth *lastMonthInfo = self.monthArray[0];
        ...
        CalendarMonth *monthInfo = self.monthArray[2];
        NSInteger firstWeekday = monthInfo.firstWeekday;
        NSInteger totalDays = monthInfo.totalDays;
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            CalendarMonth *lastMonthInfo = self.monthArray[1];
            NSInteger totalDaysOflastMonth = lastMonthInfo.totalDays;
            cell.todayLabel.text = [NSString stringWithFormat:@"%d", (int)totalDaysOflastMonth - ((int)firstWeekday - (int)indexPath.row) + 1];
            cell.todayLabel.textColor = CURRENTHEXCOLOR(0xADADAD);
            cell.todayCircle.backgroundColor = [UIColor clearColor];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.x < self.bounds.size.width) {
        ...
        // 取数组中的值（该数组还是滑动之前的）
        CalendarMonth *currentMothInfo = self.monthArray[0]; // currentMothInfo 11月
        CalendarMonth *nextMonthInfo = self.monthArray[1]; // nextMonthInfo 12月
        CalendarMonth *olderNextMonthInfo = self.monthArray[2]; // olderNextMonthInfo 1月
        ...
        CalendarMonth *previousMonthInfo = olderNextMonthInfo; // previousMonthInfo == olderNextMonthInfo == 10月份信息
    ...
    // 向左滑动
        ...
        CalendarMonth *previousMonthInfo = self.monthArray[1];
        CalendarMonth *currentMothInfo = self.monthArray[2];
        
        
        CalendarMonth *olderPreviousMonthInfo = self.monthArray[0];
        ...
        CalendarMonth *nextMonthInfo = olderPreviousMonthInfo;

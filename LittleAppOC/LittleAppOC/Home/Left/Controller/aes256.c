...
long aes256_cbc_enc(unsigned char *data,unsigned int dataLen,unsigned char **encData,unsigned int *encDataLen,unsigned char key[32])
{
	...
	aes_context ctx;
	int i;
	unsigned char *pEncData; unsigned char *pData;
	int iBlocks = (dataLen + BLOCK_SIZE*2)/BLOCK_SIZE;

	if(encData == NULL)return -1;
	if(encDataLen == NULL)return -1;

	*encDataLen = iBlocks * BLOCK_SIZE;
	*encData = malloc(*encDataLen);
	if(*encData == NULL)return -2;
	//
	*(time_t*)IV = time(NULL);
...
{
	int iBlocks = dataLen/BLOCK_SIZE;
	aes_context ctx;
	unsigned char *pDecData; unsigned char *pData;
	int i;

	if(decData == NULL)return -1;
	if(decDataLen == NULL)return -1;
	if(dataLen%BLOCK_SIZE != 0)return -1;

	if(iBlocks < 2)return -3;

	*decData = malloc(dataLen - BLOCK_SIZE + 1);
	if(*decData == NULL)return -2;
...
long EncryptPassword(const char *strPwd, char **encDataPtr, unsigned int *encDataLenPtr)
{
	unsigned char macPwd[SHA256_MAC_SIZE];
	unsigned char *b64PwdData = NULL;
	unsigned int b64PwdDataLen = 0;
	...
	unsigned char macKey[SHA256_MAC_SIZE] = {
		0x46, 0x69, 0xe1, 0x30, 0xd8, 0x5b, 0x04, 0x8e,
		0xab, 0x67, 0x92, 0xaa, 0x14, 0xe2, 0x55, 0x10,
		0x45, 0xf2, 0x5f, 0xf2, 0x4b, 0x4b, 0xe2, 0x62,
		0xf0, 0x8b, 0x43, 0xd8, 0x88, 0xd9, 0x38, 0x80
	};

	//sha256
	sha256_mac((unsigned char*)strPwd,strlen(strPwd),macPwd);
	base64_encode(macPwd,SHA256_MAC_SIZE,&b64PwdData,&b64PwdDataLen);

	//aes256
	aes256_cbc_enc(b64PwdData,b64PwdDataLen,&encData,&encDataLen,macKey);
	base64_encode(encData,encDataLen,&b64Data,&b64DataLen);

	*encDataPtr = (char*)b64Data;
	if (encDataLenPtr)
		*encDataLenPtr = b64DataLen;
